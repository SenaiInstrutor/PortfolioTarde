...
namespace Calculadora_Basica
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Digite a operação matemática desejada: ( /= divisão , * = multiplicação, + = soma, - = subtração e % = MOD resto ) em seguida aperte enter: ");
            operador = Console.ReadLine();

            

            Console.WriteLine("Digite o segundo valor, em seguida aperte enter: ");
            valorDigitado = Console.ReadLine();
            numero_inteiro = double.TryParse(valorDigitado, out numero);
            if (numero_inteiro)
            {
